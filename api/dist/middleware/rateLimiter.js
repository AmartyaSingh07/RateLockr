...
async function recordRequestEvent(clientId, allowed) {
    ...
    const counterKey = allowed ? (0, keys_1.statsAllowKey)(clientId) : (0, keys_1.statsDenyKey)(clientId);
    try {
    ...
    try {
        ...
        if (!result.allowed) {
            ...
            recordRequestEvent(resolvedClientId, false).catch((err) => {
                logger_1.logger.error({ err }, "Failed to record deny event");
            });
        ...
        recordRequestEvent(resolvedClientId, true).catch((err) => {
            logger_1.logger.error({ err }, "Failed to record allow event");
        });
        next();
    }
    catch (err) {
